...
void g_AddLinkIntoSelectionList(int link_no, int document_no, bool b_SelectOtherDocuments, double x, double y)
{
	...
	while (iDoc != g_DocumentList.end())
	{
		...
		point.x = x;
		point.y = y;
		int sel_link_no = (*iDoc)->SelectLink(point);
...
bool CTLiteDoc::ReadNodeCSVFile(LPCTSTR lpszFileName, int LayerNo)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		...
		for (std::list<DTANode*>::iterator iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
		{
			...
			m_GridMatrix[x_key][y_key].m_NodeVector .push_back ((*iNode)->pt.x );
			m_GridMatrix[x_key][y_key].m_NodeVector .push_back ((*iNode)->pt.y );
...
int CTLiteDoc::FindClosestNode(double x, double y)
{
	int x_key = (x - m_GridRect.left)/ m_GridXStep;
	int y_key = (y -m_GridRect.bottom)/ m_GridYStep;
	...
	for(int x_i = max(0,x_key-1); x_i <= min(99,x_key+1); x_i++)
		for(int y_i = max(0,y_key-1); y_i <= min(99,y_key+1); y_i++)
		{
			...
			for(unsigned int i = 0; i < element.m_NodeVector.size(); i++)
			{
				...
				double distance = sqrt( (x-element.m_NodeX[i])*(x-element.m_NodeX[i]) + (y-element.m_NodeX[i])*(y-element.m_NodeX[i]));
...
void CTLiteDoc::LoadSimulationOutput()
{
	...
	if(ReadSimulationLinkMOEData_Bin(m_ProjectDirectory+"output_LinkTDMOE.bin")==false)
	{
	ReadDYNASMARTSimulationResults(); // try to read DYNASMART simulation results if output_LinkTDMOE.bin is not available
...
int CTLiteDoc::SelectLink(GDPoint point)
{
...
bool CTLiteDoc::ReadCoordinateInfoFromNodeCSVFile(LPCTSTR lpszFileName)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		...
		while(parser.ReadRecord())
		{
			...
			if( m_NodeNumberMap.find(node_id) != m_NodeNumberMap.end())
			{
				DTANode* pNode = m_NodeNumberMap[node_id];
				pNode->pt.x = X;
				pNode->pt.y = Y;
			//			cout << "node = " << node << ", X= " << X << ", Y = " << Y << endl;
...
bool CTLiteDoc::ReadCoordinateInfoFromLinkCSVFile(LPCTSTR lpszFileName)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		...
		while(parser.ReadRecord())
		{
			...
			DTALink* pLink =  FindLinkWithNodeIDs(m_NodeNametoIDMap[from_node_id],m_NodeNametoIDMap[to_node_id]);
