		...
		while(parser.ReadRecord())
		{
			...
			if(pLink!=NULL)
			{
				if(t < g_Simulation_Time_Horizon)
				{
					...
					//				parser.GetValueByFieldName("delay_in_min",pLink->m_LinkMOEAry[t].TravelTime);
					//parser.GetValueByFieldName("link_volume_in_veh_per_hour_per_lane",pLink->m_LinkMOEAry[t].LinkFlow);
					parser.GetValueByFieldName("link_volume_in_veh_per_hour_for_all_lanes",pLink->m_LinkMOEAry[t].LinkFlow);
					parser.GetValueByFieldName("density_in_veh_per_mile_per_lane",pLink->m_LinkMOEAry[t].Density );
					parser.GetValueByFieldName("speed_in_mph",pLink->m_LinkMOEAry[t].Speed);
	...
	if(pFile!=NULL)
	{
		...
		while(!feof(pFile))
		{
			...
			{	
					...
					if(element.travel_time_in_min<-1)
					{
					TRACE("");
					}
					//				parser.GetValueByFieldName("delay_in_min",pLink->m_LinkMOEAry[t].TravelTime);
					//parser.GetValueByFieldName("link_volume_in_veh_per_hour_per_lane",pLink->m_LinkMOEAry[t].LinkFlow);
					pLink->m_LinkMOEAry[t].LinkFlow = element.link_volume_in_veh_per_hour_for_all_lanes;
					...
					//pLink->m_LinkMOEAry[t].number_of_through_and_right_queued_vehicles = element.number_of_through_and_right_queued_vehicles;
...
bool CTLiteDoc::ReadSimulationLinkOvarvallMOEData(LPCTSTR lpszFileName)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		...
		while(parser.ReadRecord())
		{
			...
			if(from_node_number == 9 && to_node_number ==10)
			{
			TRACE("");
			}
			DTALink* pLink = FindLinkWithNodeNumbers(from_node_number , to_node_number, lpszFileName );
...
bool CTLiteDoc::ReadActivityLocationCSVFile(LPCTSTR lpszFileName)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		...
		while(parser.ReadRecord())
		{
			...
			if(m_ZoneMap.find (zone_number)== m_ZoneMap.end ())
			{
...
bool CTLiteDoc::ReadScenarioSettingCSVFile(LPCTSTR lpszFileName)
{
	...
	if (parser_scenario.OpenCSVFile(lpszFileName))
	{
		...
		while(parser_scenario.ReadRecord())
		{
			...
			parser_scenario.GetValueByFieldName("number_of_statistics_reporting_days",m_number_of_reporting_days);
...
bool CTLiteDoc::WriteScenarioSettingCSVFile(LPCTSTR lpszFileName)
{
	...
	if(m_NumberOfSecenarioSettings==1)
	{
		...
		if(ScenarioFile.Open(lpszFileName))
		{
			...
			ScenarioFile.SetFieldNameAndValue("number_of_statistics_reporting_days",m_number_of_reporting_days);//
...
BOOL CTLiteDoc::SaveLinkData(LPCTSTR lpszPathName,bool bExport_Link_MOE_in_input_link_CSF_File, int SelectedLayNo)
{
	...
	{
		std::list<DTALink*>::iterator iLink;
		fprintf(st,"name,link_id,link_key,TMC,from_node_id,to_node_id,link_type_name,direction,length,number_of_lanes,speed_limit,lane_capacity_in_vhc_per_hour,link_type,jam_density,wave_speed,mode_code,grade,geometry,original_geometry,");
		...
		for (iLink = m_LinkSet.begin(); iLink != m_LinkSet.end(); iLink++)
		{
			if((*iLink)->m_LayerNo == SelectedLayNo && (*iLink)->m_FromNodeID != (*iLink)->m_ToNodeID)
			{
				...
				fprintf(st,"%s,%d,%s,%s,%d,%d,%s,%d,%.5f,%d,%.1f,%.1f,%d,%.1f,%.1f,\"%s\",%.1f,",
					(*iLink)->m_Name.c_str (),
					(*iLink)->m_LinkID, 
					(*iLink)->m_LinkKey , 
				...
				fprintf(st,"%.4f,%.4f,",
				(*iLink)->m_BPR_alpha_term, (*iLink)->m_BPR_beta_term);
				...
				{
					DTALink * pLine  = m_NodeNotoLinkMap[ReversedLinkKey];
					if(pLine!= NULL)
...
BOOL CTLiteDoc::SaveProject(LPCTSTR lpszPathName, int SelectedLayNo)
{
	...
	{
		std::list<DTANode*>::iterator iNode;
		fprintf(st, "name,node_id,QEM_reference_node_id,control_type,control_type_name,x,y,geometry\n");
		for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
		{
			if((*iNode)->m_LayerNo == SelectedLayNo) 
			{
				...
				fprintf(st, "%s,%d,%d,%d,%s,%f,%f,\"<Point><coordinates>%f,%f</coordinates></Point>\"\n", 
...
//	SaveInputLanesFile(directory+"input_lanes.csv");
	SaveMovementData(directory+"AMS_movement.csv", -1);
	...
	//					DTANodeMovement movement = (*iNode)->m_MovementDataMap["FREE"].m_MovementVector[movement_index];
	...
	CMainFrame* pMainFrame = (CMainFrame*) AfxGetMainWnd();
		...
		WritePrivateProfileString("initialization", "UserDefinedSimulator",theApp.m_SimulatorString ,NEXTASettingsPath);
...
bool CTLiteDoc::SaveInputPathCSVFile(LPCTSTR lpszFileName)
{
	...
	if(st!=NULL)
	{
		...
		{
		CString str = 		GetTimeStampStrFromIntervalNo (min,true);
		fprintf(st,"%s,",str);
		}
...
void CTLiteDoc::ScanAMSTimingPlanCSVFile(LPCTSTR lpszFileName, int scenario_no = 0)
{
	...
	//reset
	m_TimingPlanNameVector.clear();

	// default value
	AddNameIntoTimingPlanVector("FREE");
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
		while(parser.ReadRecord())
		{
			...
			for(int time = 0; time <1440; time+=15)
			{
				...
				{
				AddNameIntoTimingPlanVector(timing_plan_name);
				}
...
int CTLiteDoc::ReadAMSMovementCSVFile(LPCTSTR lpszFileName, int NodeNumber = -1)
{
			...
			for(unsigned int m = 0; m< (*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector .size(); m++)
			{
				...
				// set default value for through turns
					...
					switch ((*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector[m].movement_turn)
						...
						case DTA_LeftTurn: 
						case DTA_LeftTurn2:
						(*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector[m].QEM_Lanes = pLink->m_NumberOfLeftTurnLanes ; break;

						
						case DTA_RightTurn:
						case DTA_RightTurn2: 
						(*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector[m].QEM_Lanes = pLink->m_NumberOfRightTurnLanes ; break;
						}
	...
	if (parser_movement.OpenCSVFile(lpszFileName))
	{
		while(parser_movement.ReadRecord())
		{
			...
			int movement_index = pNode->GetMovementIndex(m_NodeNumbertoNodeNoMap[up_node_id],
	...
	for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
	{
		if((*iNode)->m_LayerNo == 0  && ((*iNode)->m_ControlType == m_ControlType_PretimedSignal || (*iNode)->m_ControlType == m_ControlType_ActuatedSignal)) 
		{
			...
			{
				if((*iNode)->m_MovementDataMap["FREE"].m_MovementVector[m].QEM_Lanes >=1 && (*iNode)->m_MovementDataMap["FREE"].m_MovementVector[m].QEM_EffectiveGreen  <=6 )
				{  // use default value

					
					DTALink* pLink0 = m_LinkNoMap[(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[m].IncomingLinkNo];


						(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[m].QEM_EffectiveGreen = 
...
void  CTLiteDoc::UpdateMovementDataFromVehicleTrajector()
{
		...
		for (iVehicle = m_VehicleSet.begin(); iVehicle != m_VehicleSet.end(); iVehicle++)
		{
			...
			for(int i= 2; i<pVehicle->m_NodeSize; i++)
			{
				...
				if(pLink0->m_FromNodeNumber == 1 && pLink0->m_ToNodeNumber == 5 && delay >5)
				{
					TRACE("delay: %f\n", delay);
				}
				...
				if(pLink0 !=NULL && pLink1!=NULL &&Hour >=0 && Hour<24)
				{
					...
					std::string timing_plan_name = "FREE";
					pNode->AddMovementCountAndDelay(Hour, pLink0->m_FromNodeID, pLink1->m_ToNodeID, delay , timing_plan_name);
				}
		...
		for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
		{
			for(unsigned int m = 0; m< (*iNode)->m_MovementDataMap["FREE"].m_MovementVector .size(); m++)
			{
				...
				for(unsigned int j = 0; j< (*iNode)->m_MovementDataMap["FREE"].m_MovementVector .size(); j++)
				{
					...
					{
						total_link_count+= (*iNode)->m_MovementDataMap["FREE"].m_MovementVector[j].sim_turn_count ;
					}
		...
		for(CurrentTime  = StartTime; CurrentTime < EndTime; CurrentTime ++)
		{
					...
					pLink->GetSimulatedDensity(CurrentTime) >=0.1)
				{
					...
					case MOE_reliability: 
						...
						break;
					case MOE_traveltime:
						total_value+= pLink->GetTravelTime (CurrentTime);
						break;
					case MOE_density:
...
void CTLiteDoc::OnToolsPerformtrafficassignment()
{
	...
	{
		sCommand = theApp.m_SimulatorString; // user defined simulator
	}
	else
	{
...
void CTLiteDoc::LoadSimulationOutput()
{
	...
	ReadSimulationLinkOvarvallMOEData(m_ProjectDirectory+"output_LinkMOE.csv");
		if(ReadSimulationLinkMOEData_Bin(m_ProjectDirectory+"output_LinkTDMOE.bin") == true)
		{
	...
	{
		if(AfxMessageBox("The simulation result files in this project/folder might be generated by a previous run with a different network.\nDo you want to remove those outdated simulation results?", MB_YESNO|MB_ICONINFORMATION)== IDYES)
		{
			CWaitCursor wait;
			DeleteFile(m_ProjectDirectory+"output_LinkMOE.csv");
			DeleteFile(m_ProjectDirectory+"output_LinkTDMOE.bin");
			DeleteFile(m_ProjectDirectory+"agent.bin");
			DeleteFile(m_ProjectDirectory+"output_agent.csv");  // finally try csv file.
...
bool CTLiteDoc::CheckControlData()
{
	...
	if(NodeMap.size()>0 && m_signal_reresentation_model ==1)
	{
		CString message;
...
void CTLiteDoc::ResetBackgroundImageCoordinate()
{
	...
	m_UnitMile = 1.0/62;  // 1 degree of long = 62 miels
...
void CTLiteDoc::OnMoeViewoddemandestimationsummaryplot()
{
	...
	CString str = m_ProjectDirectory +"export_validation_results.csv";
	...
	if (parser_test.OpenCSVFile(strStd)==false)  // no such file
	{
	...
	if (parser.OpenCSVFile(strStd))
	{
		...
		if(m_sensor_data_aggregation_type == 0)
		{
		...
		if(m_sensor_data_aggregation_type == 1)
		{
			element_link_volume.szName  = "Hourly link volume";
			dlg.m_XCaption = "Observed Hourly Link Volume";
			dlg.m_YCaption = "Simulated Hourly Link Volume";
		}

		if(m_sensor_data_aggregation_type == 2)
		{
			element_link_volume.szName  = "Hourly lane volume";
			dlg.m_XCaption = "Observed Hourly Lane Volume";
			dlg.m_YCaption = "Simulated Hourly Lane VVolume";
		}
		...
		while(parser.ReadRecord())
		{
			...
			float observed_link_count  = 0;
			if(parser.GetValueByFieldName("observed_link_count",observed_link_count) == false)
				break;
			...
			if(observed_link_count>0.1f)   // with sensor data
			{

				float simulated_link_count  = 0;
				if(parser.GetValueByFieldName("simulated_link_count",simulated_link_count) == false)  // estimated data
					break;

				if(m_sensor_data_aggregation_type == 0)
				{
					data.x = observed_link_count;
					data.y = simulated_link_count;
				}

				float hour = (end_time_in_min - start_time_in_min )/60;

				if(hour < 0.1) hour = 0.1;

				if(m_sensor_data_aggregation_type == 1)
				{
					data.x = observed_link_count / hour;
					data.y = simulated_link_count/ hour;
				}

				if(m_sensor_data_aggregation_type == 2)
				{
					data.x = observed_link_count/ hour / max(1,pLink->m_NumberOfLanes) ;
					data.y = simulated_link_count/hour / max(1,pLink->m_NumberOfLanes) ;
				}
		...
		{
			AfxMessageBox("No matching sensor data are available from file export_validation_results. Please re-generate file export_validation_results.");
		}
	}else
	{
		AfxMessageBox("File export_validation_results.csv does not exist. Please first run traffic assignment.");
	}
...
void CTLiteDoc::OnToolsGeneratesignalcontrollocations()
{
	...
	CString msg;
	msg.Format("%d nodes are assigned to signal control type.", signal_count);
	AfxMessageBox(msg,MB_ICONINFORMATION);
...
void CTLiteDoc::ReadTMCSpeedData(LPCTSTR lpszFileName)
{
	...
	if (parser.OpenCSVFile(lpszFileName))
	{
			...
			if(m_TMC2LinkMap.find(TMC)!=m_TMC2LinkMap.end())
			{
				...
				for (int t = 0; t< 1440; t+=15)
				{
					...
					if(parser.GetValueByFieldName(StdString,speed_in_mph) != false)
					{
						...
						for(int s= 0 ; s<15; s++)
						{
							pLink->m_LinkMOEAry[t+s].LinkFlow = 1000; // default hourly volume

							pLink->m_LinkMOEAry[t+s].Speed = speed_in_mph;
						}
		...
		m_SensorDataLoadingStatus.Format ("%d TMC records are loaded from file %s.\n",i,lpszFileName);
...
void CTLiteDoc::RunQEMTool(int NodeNumber = -1)
{
	...
	SaveMovementData(m_ProjectDirectory + "AMS_movement.csv", -1);
	SaveMovementData(m_ProjectDirectory + "AMS_movement.input.csv", -1);  // as back up file

	SaveAMSPhasingData(m_ProjectDirectory+"AMS_phasing.csv");
	SaveAMSPhasingData(m_ProjectDirectory+"AMS_phasing.input.csv"); // as back up
	...
	DTA_Phasing_Data_Matrix element = GetPhaseData(NodeNumber ,"FREE");
...
void CTLiteDoc::SaveMovementData(CString MovementFileName, int NodeNumber = -1)
{
	...
	if(MovementFile.Open(strStd))
	{
		...
		MovementFile.SetFieldName ("CycleLength");
		MovementFile.SetFieldName ("Offset");
		...
		for(int tp = 0; tp< m_TimingPlanNameVector.size(); tp++)  // first loop for each timing plan
		{
			...
			if((*iNode)->m_MovementDataMap.find(timing_plan_name) != (*iNode)->m_MovementDataMap.end() && (*iNode)->IfTrafficExistInThisTimingPlan(timing_plan_name) == true)
			{
			for(unsigned int m = 0; m< (*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector .size(); m++)
			{
				...
				(*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector[m].QEM_TurnVolume  =  (*iNode)->m_MovementDataMap[timing_plan_name].m_MovementVector[m].sim_turn_hourly_count ;
				...
				MovementFile.SetValueByFieldName  ("CycleLength",(*iNode)->m_CycleLengthInSecond );
...
void CTLiteDoc::SaveQEMMovementData(CString MovementFileName, bool bSimulatedCountFlag)
{
	...
	std::vector<DTA_SIG_MOVEMENT> Approach_vector;
	...
	if(st!=NULL)
	{
		...
		for (iNode = m_NodeSet.begin(); iNode != m_NodeSet.end(); iNode++)
		{
			if((*iNode)->m_MovementDataMap["FREE"].m_MovementVector .size() > 0)
			{
				...
				for(unsigned i= 0; i < Approach_vector.size(); i++)
				{
					...
					if(MovementIndex >=0)
					{  
						fprintf(st,"%d,",(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex]. QEM_Lanes);
				...
				for(unsigned i= 0; i < Approach_vector.size(); i++)
				{
					...
					{  
						if(bSimulatedCountFlag) 
							fprintf(st,"%.1f,",(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex]. sim_turn_hourly_count);
						else // observed 
							fprintf(st,"%d,",(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex]. obs_turn_hourly_count);
				...
				for(unsigned i= 0; i < Approach_vector.size(); i++)
				{
					...
					if(MovementIndex >=0)
					{  
						fprintf(st,"%.0f,",(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex].QEM_Speed);
				...
				for(unsigned i= 0; i < Approach_vector.size(); i++)
				{
					...
					{  
						...
						DTALink* pLink = FindLinkWithNodeIDs(
							(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex].in_link_from_node_id,
							(*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex].in_link_to_node_id);
				...
				for(unsigned i= 0; i < Approach_vector.size(); i++)
				{
					...
					{  
						...
						fprintf(st,"%.1f,", (*iNode)->m_MovementDataMap["FREE"].m_MovementVector[MovementIndex].QEM_EffectiveGreen);
...
void CTLiteDoc::OnLinkAddRadioMessage()
{
	...
	if(pLink!=NULL)
	{
		...
		CDlgScenario dlg(_RADIOMESSAGE);
		dlg.m_pDoc = this;
...
void CTLiteDoc::OnSensortoolsImportsensordatafromimport()
{
	...
	if (parser.OpenCSVFile(CString2StdString(SensorFileName)))
	{
		...
		FILE* st;
		fopen_s(&st,m_ProjectDirectory+"input_sensor.csv","w");
		if(st!=NULL)
		{
		}else
		{
			AfxMessageBox("File input_sensor.csv cannot be opened to write.");
			return;
		}
	...
	CString Message;
	Message.Format("%d sensor records have been processed into file input_sensor.csv.",data_count);
...
void CTLiteDoc::GenerateMovementShapePoints()
{
	...
	for (unsigned int i=0;i< pNode->m_MovementDataMap["FREE"].m_MovementVector .size();i++)
	{
		DTANodeMovement movement = pNode->m_MovementDataMap["FREE"].m_MovementVector[i];
		 ...
		 pNode->m_MovementDataMap["FREE"].m_MovementVector[i].m_ShapePoints .clear ();
		...
		// 		GDPoint p1_new, p2_new, p3_new;
		pNode->m_MovementDataMap["FREE"].m_MovementVector[i].m_ShapePoints.push_back(p1_new);
		pNode->m_MovementDataMap["FREE"].m_MovementVector[i].m_ShapePoints.push_back(p2_new);
		pNode->m_MovementDataMap["FREE"].m_MovementVector[i].m_ShapePoints.push_back(p3_new);
...
void CTLiteDoc::OnMovementSetupnumberofleftturnlanesforsignalizednodes()
{
 ...
 //   dlg.m_TimingPlanStringVector =   m_TimingPlanNameVector;
	...
	//	int SelectedTimingPlanNo = dlg.m_SelectedTimingPlanNo;
	...
	//				DTA_Phasing_Data_Matrix element_current = GetPhaseData((*iNode)->m_NodeNumber ,  m_TimingPlanNameVector[SelectedTimingPlanNo]);

	//				if(element_current.GetValue (
