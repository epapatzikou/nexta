...
//read trip file
	...
	if (parser.OpenCSVFile(strStd5))
	{
		...
		while(parser.ReadRecord())
		{
			PT_shapes Tripshapes;
            //GDPoint point;
			
			if(parser.GetValueByFieldName("shape_id",Tripshapes.shape_id) == false)
				break;	

			if(parser.GetValueByFieldName("shape_pt_lat",Tripshapes.shape_pt_lat) == false)
				break; 			
			
			if(parser.GetValueByFieldName("shape_pt_lon",Tripshapes.shape_pt_lon) == false)
				break;

			if(parser.GetValueByFieldName("shape_pt_sequence",Tripshapes.shape_pt_sequence) == false)
				break;	
			...
			m_PT_shapes.push_back(Tripshapes) ;
   ...
   for ( iPT_TripMap=m_PT_TripMap.begin() ; iPT_TripMap != m_PT_TripMap.end(); iPT_TripMap++ )
	 {
		...
		{
          //find shape line point on the route by the (route_id) the route----(trip.id,route.id,shape.id)trip table--(shape_id)shapes table
           for ( iPT_shapes=m_PT_shapes.begin() ; iPT_shapes != m_PT_shapes.end(); iPT_shapes++ )
		   {
