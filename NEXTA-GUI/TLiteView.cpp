...
void CTLiteView::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	//	if(pow((size.cx*size.cx + size.cy*size.cy),0.5) < m_NodeSize*2)
	{
		...
		if(	(pMainFrame->m_iSelectedLayer == layer_link || pMainFrame->m_iSelectedLayer == layer_link_MOE ) &&
...
void CTLiteView::DrawLinkAsLine(DTALink* pLink, CDC* pDC)
{
	...
	if(pDoc->m_LinkMOEMode == MOE_queue_length)   // queue length mode
	{
		...
		float value;
		float percentage = pDoc->GetLinkMOE(pLink, pDoc->m_LinkMOEMode,(int)g_Simulation_Time_Stamp,g_MOEAggregationIntervalInMin, value);
