...
int g_ODEstimation_SensorDayNo = 0;
			...
			for(unsigned int i = 0; i< pLink->m_LinkMeasurementAry.size(); i++)
			{
				...
				if(SimulatedInFlowCount>=0)
				{
					...
					if(pLink->m_LinkMeasurementAry[i].ObsTravelTime>=0.01)
					{
						...
						float SpeedError = (pLink->m_Length / max(0.01, pLink->GetSimulatedTravelTime(StartTime)) * 60 - pLink->m_Length / max(0.01, pLink->m_LinkMeasurementAry[i].ObsTravelTime) * 60);
						float AbosolutePercentageError = abs(SpeedError) / (pLink->m_Length / max(0.01, pLink->m_LinkMeasurementAry[i].ObsTravelTime) * 60)*100;
