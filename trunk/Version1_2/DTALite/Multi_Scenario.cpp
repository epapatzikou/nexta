	...
	if (parser_scenario.OpenCSVFile("input_scenario_settings.csv"))
	{
		...
		if(g_UEAssignmentMethod == assignment_day_to_day_learning_threshold_route_choice )
			...
			if(g_ODEstimationFlag ==1)
			{
				...
				if (parser_scenario.GetValueByFieldName("ODME_sensor_day_no", g_ODEstimation_SensorDayNo) == false)
				{
					g_ODEstimation_SensorDayNo = 0;
					cout << "Field g_ODEstimation_SensorDayNo has not been specified in file input_scenario_settings.csv. A default factor of 0 is used." << endl;
				}
