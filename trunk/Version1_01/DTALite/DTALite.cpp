		...
		for (unsigned li = 0; li< g_LinkVector.size(); li++)
		{
			...
			for (int time = g_DemandLoadingStartTimeInMin; time <= g_SimululationReadyToEnd; time++)
			{
				...
				if (time < pLink->m_LinkMOEAry.size())
				{
					...
					fprintf(st, "%d,%d,%d->%d,%d,%d,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%3.2f,%3.2f,%d,%d,%d,%5.1f,",
						...
						pLink->m_LinkMOEAry[time].SystemOptimalMarginalCost );
...
void g_FreeMemory(bool exit_flag = true)
{
	...
	if (g_TDOVehicleArray != NULL && g_ODZoneNumberSize > 0);
	{
		...
		g_AggregationTimetIntervalSize = -1;
	}
