...
bool g_ReadLinkMeasurementFile()
{
	...
	if (parser.OpenCSVFile("input_sensor.csv"))
	{
		...
		while(parser.ReadRecord())
		{
			...
			{
				cout << "Link " << name <<":" << FromNodeNumber << "-> " << ToNodeNumber << " at line " << count+1 << " of file input_sensor.csv  has not been defined in input_link.csv. Please check." << endl;
				g_LogFile << "Link " << name <<":" << FromNodeNumber << "-> " << ToNodeNumber << " at line " << count+1 << " of file input_sensor.csv  has not been defined in input_link.csv. Please check." << endl;
				//if(error_count<=3)
				//{
				//	getchar();
				//}
			...
			string sensor_type;
			parser.GetValueByFieldNameRequired("sensor_type",sensor_type );
			...
			if(pLink!=NULL)
			{
				...
				if(sensor_type.find("link_count")!= string::npos)
				{
				...
				int dest_node_id = -1;
				if(sensor_type.find("movement_count")!= string::npos)
				{
				parser.GetValueByFieldName ("movement_count",volume_count );
				if(!parser.GetValueByFieldName("dest_node_id",dest_node_id )) 
				{
				cout << "missing dest_node_id for link " << name <<":" << FromNodeNumber << "-> " << ToNodeNumber << " at line " << count+1 << " of file input_sensor.csv  has not been defined in input_link.csv. Please check." << endl;
				getchar();
				}
				}
				...
				if(sensor_type.find("movement_count")!= string::npos)
					element.DestinationNode = dest_node_id;
	...
	}else
	{
		...
		g_ProgramStop();
			...
			if(g_ODEstimationEndTimeInMin < g_DemandLoadingStartTimeInMin)
			{
				cout << "Error: ODME end time ( " << g_ODEstimationEndTimeInMin << ")> demand loading end time (" << g_DemandLoadingStartTimeInMin << ")" << endl;
				g_ProgramStop();
			}
...
{  // this function has been enhanced for path flow adjustment
	...
	{
		if(g_ZoneIDVector[DestZoneID]<0)  // no such zone number
			continue;
	...
	for (int vi = 0; vi<g_TDOVehicleArray[origin_zone][AssignmentInterval].VehicleArray.size(); vi++)
	{
		...
		{
			g_EstimationLogFile << "Critical vehicle: " <<  VehicleID << endl;

			if(VehicleID == 3759)
				cout << "tracking vehicle id " << VehicleID << endl;
		}
		...
		if(PathArray[VehicleDest].NumOfPaths == 0) // the first path for VehicleDest
		{
			...
			if(PathIndex == 0) // a new path is found
			{
				...
				if(PathArray[VehicleDest].NumOfPaths>= _MAX_ODT_PATH_SIZE_4_ODME-1) 
				{
					cout << " Too many paths for ODME: " <<  PathArray[VehicleDest].NumOfPaths << " > " << _MAX_ODT_PATH_SIZE_4_ODME << endl;
 					g_ProgramStop();
				}
				PathArray[VehicleDest].NumOfVehsOnEachPath[PathArray[VehicleDest].NumOfPaths]++;
				PathArray[VehicleDest].PathNodeSums[PathArray[VehicleDest].NumOfPaths] = NodeSum;
	...
	for(DestZoneID=1; DestZoneID <= g_ODZoneNumberSize; DestZoneID++)
	{
		if(PathArray[DestZoneID].NumOfPaths > 0)
		{
			...
			for(p=1; p<=PathArray[DestZoneID].NumOfPaths; p++)
			{
				...
				if(origin_zone == CriticalOD_origin && DestZoneID == CriticalOD_destination)
				{
					g_EstimationLogFile << "Critical OD demand " << origin_zone << " -> " << DestZoneID << " path " << p << " gap: " << PathArray[DestZoneID].AvgPathGap[p] << endl;
				}

				// we calculate the gap for the previous iteraiton, for each path, path travel time gap * path flow
// comment out for omp				g_CurrentGapValue += (PathArray[DestZoneID].AvgPathGap[p]* PathArray[DestZoneID].NumOfVehsOnEachPath[p]);
	...
	for(DestZoneID=1; DestZoneID <= g_ODZoneNumberSize; DestZoneID++)  //  for each OD pair
	{

		if(g_ZoneIDVector[DestZoneID]<0)  // no such Zone ID
			continue;
		...
		float hist_demand = g_HistDemand.GetValue (origin_zone, DestZoneID, AssignmentInterval);
		...
		if(origin_zone == CriticalOD_origin && DestZoneID == CriticalOD_destination)
		{
			g_EstimationLogFile << "Critical OD demand " << origin_zone << " -> " << DestZoneID <<  " with "<< PathArray[DestZoneID].NumOfPaths << " paths; Hist Demand =" << hist_demand << "; simu Demand = " << PathArray[DestZoneID].NumOfVehicles  << ";Dev = " << PathArray[DestZoneID].DeviationNumOfVehicles <<endl; 
		}
		...
		for(int p=1; p<= PathArray[DestZoneID].NumOfPaths; p++)  // recall that path index start from 1 in this module
		{
			...
			while(l>=0 && l< PathArray[DestZoneID].PathSize[p]-1)  // for each link along the path
			{
				...
				DTALink* pLink = g_LinkVector[LinkID];
						...
						if(origin_zone == CriticalOD_origin && DestZoneID == CriticalOD_destination && ArrivalTime>=990)
						{
							g_EstimationLogFile << "Link Index : " << l << "Critical OD demand " << origin_zone << " -> " << DestZoneID << " passing link " << pLink->m_FromNodeNumber << "->" << pLink->m_ToNodeNumber << ", arrival time = "<< ArrivalTime << endl;
						}
				...
				if(pLink->m_LinkMOEAry [(int)(ArrivalTime)].TrafficStateCode == 0) 					// if Arrival at Free-flow condition.
				{
					...
					if(g_GetSequentialEstimationTimePeriod(iteration,ArrivalTime) && pLink->ContainFlowCount(ArrivalTime)) // with flow measurement
					{
						...
						if(origin_zone == CriticalOD_origin && DestZoneID == CriticalOD_destination)
						{
							g_EstimationLogFile << "Critical OD demand " << origin_zone << " -> " << DestZoneID << " passing link " << pLink->m_FromNodeNumber << "->" << pLink->m_ToNodeNumber 
...
//					cout << "ODME: reset adjustment flow to 0: avoid heavy congestion." << endl;
			...
			*/
			...
			element.Setup (PathIndex,origin_zone, DestZoneID, 1, AssignmentInterval *g_AggregationTimetInterval, (AssignmentInterval+1) *g_AggregationTimetInterval,
				PathArray[DestZoneID].PathSize[p]-1, PathArray[DestZoneID].PathLinkSequences[p],PathArray[DestZoneID].NewNumberOfVehicles[p],PathArray[DestZoneID].PathNodeSums[p],AssignmentInterval );
...
// for vehicles starting from departure_time_begin to departure_time_end, assign them to shortest path using a proportion according to MSA or graident-based algorithms
{
	...
	if(g_ZoneIDVector[origin_zone]<0)  // no such zone number
			return;
...
//	g_EstimationLogFile << "g_GenerateVehicleData_ODEstimation "  <<endl; 
	...
	for(int z = 0; z <= g_ODZoneNumberSize; z++)
	{
		...
		if(g_ZoneIDVector[z]<0)  // no such Zone ID
			continue;
		...
		for(int di = 0; di < g_AggregationTimetIntervalSize; di++)
		{
			...
			for(int pi = 0; pi< g_TDOVehicleArray[z][di].m_ODTKPathVector .size(); pi++)
			{
	...
	{	
		...
		if(g_TrafficFlowModelFlag !=0)  // dynamic  traffic assignment
		{
			...
			for(unsigned int i = 0; i< pLink->m_LinkMeasurementAry.size(); i++)
			{
				...
				if( pLink->m_LinkMeasurementAry[i].EndTime  < g_ODEstimationStartTimeInMin || pLink->m_LinkMeasurementAry[i].StartTime > g_ODEstimationEndTimeInMin)
					continue;
				...
				int SimulatedFlowCount = pLink->m_LinkMOEAry[EndTime].CumulativeArrivalCount - pLink->m_LinkMOEAry[ StartTime ].CumulativeArrivalCount; 
				...
				if(ObsFlowCount >= 1)  // flow count
				{
					...
					float AbosolutePercentageError = abs((SimulatedFlowCount -  ObsFlowCount)*1.0f/max(1,ObsFlowCount)*100);
